...
test_that("rfsrc classifications",{
  ...
  expect_is(v.obj <- rfsrc(Surv(time, status) ~ ., data = veteran, ntree = 100),
            "rfsrc")
  
  # print and plot the grow object
  expect_output(print(v.obj), "Sample size: 137")
  
  expect_is(find.interaction(v.obj),
            "matrix")
  
  expect_is(plot.variable(v.obj, surv.type="surv", time=1),
            "rfsrc")
  expect_is(plot.variable(v.obj, surv.type="surv", time=1,
                          partial=TRUE, nvar=3),
            "rfsrc")
  ...
  expect_output(plot(v.obj), "Importance")
